    ...
    create: protectedProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            return {
                ...
                previewUrl: await getSandboxPreviewUrl(newSandbox.id, template.port),
            };
        }),
        ...
        .mutation(async ({ input }) => {
            ...
            for (let attempt = 1; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
                try {
                    ...
                    const previewUrl = await getSandboxPreviewUrl(sandbox.id, input.sandbox.port);
        ...
        .mutation(async ({ input }) => {
            ...
            for (let attempt = 1; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
                try {
                    ...
                    const previewUrl = await getSandboxPreviewUrl(sandbox.id, DEFAULT_PORT);
