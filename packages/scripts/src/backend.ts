...
const tryExtractKeysFromStatus = async (): Promise<BackendKeys | null> => {
    ...
    return new Promise((resolve) => {
        ...
        const cleanup = () => {
            statusProc.stdout?.off('data', onData);
            statusProc.stderr?.off('data', onData);
            statusProc.off('close', onClose);
            statusProc.off('error', onError);
        };
...
const startBackendAndExtractKeys = async (): Promise<BackendKeys> => {
    const existingKeysSpinner = ora('Checking existing Supabase status...').start();
    const existingKeys = await tryExtractKeysFromStatus();
    if (existingKeys) {
        existingKeysSpinner.succeed('Supabase is already running.');
        return existingKeys;
    }
    existingKeysSpinner.info('Supabase not running yet. Starting...');
    ...
    const startSucceeded = await new Promise<boolean>((resolve) => {
        const timeout = setTimeout(() => {
            startProc.kill();
    ...
    if (startSucceeded) {
        ...
        spinner.warn('Supabase start did not report success. Will try to read keys from status anyway...');
    }

    // Now get all keys from status
    const keysSpinner = ora('Extracting Supabase keys...').start();
    const keys = await tryExtractKeysFromStatus();
    if (keys) {
        keysSpinner.succeed('Successfully extracted Supabase keys.');
        return keys;
    }
    keysSpinner.fail('Failed to extract Supabase keys.');
